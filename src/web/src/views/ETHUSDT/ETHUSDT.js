...
const Dashboard = () => {
  ...
  const [coinSymbol, setCoinSymbol] = useState("ETHUSDT");
  const [comparisonSymbol, setComparisonSymbol] = useState("BTCUSDT");
  ...
  const fetchAllData = async () => {
    try {
      ...
      // Get latest data for ETHUSDT
      const btcData = data.filter((coin) => coin.symbol === "ETHUSDT");
      setCoinData(btcData);
      if (btcData.length) {
  ...
  const fetchCorrelationData = async () => {
    try {
      const response = await fetch(
        "http://localhost:5001/correlation_matrix/ETHUSDT"
      );
      const data = await response.json();
  ...
  const logo = "https://s2.coinmarketcap.com/static/img/coins/64x64/1027.png";
  ...
  return (
    <CRow>
      <CRow>
        ...
        <CCol sm={8}>
          <PriceChart coinSymbol="ETHUSDT" coinData={coinData} />
        </CCol>
      </CRow>
          ...
          <CCard>
            <CCardBody>
              <div>
                ...
                <ClusterChart symbol="ETHUSDT" year={selectedYear} />
              </div>
            </CCardBody>
          </CCard>
        </CCol>
