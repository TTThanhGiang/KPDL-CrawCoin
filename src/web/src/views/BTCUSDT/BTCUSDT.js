...
const Dashboard = () => {
  ...
  const fetchAllData = async () => {
    try {
      ...
      // Get latest data for BTCUSDT
      const btcData = data.filter((coin) => coin.symbol === "BTCUSDT");
      setCoinData(btcData);
      if (btcData.length) {
  ...
  const fetchCorrelationData = async () => {
    try {
      const response = await fetch(
        "http://localhost:5001/correlation_matrix/BTCUSDT"
      );
      const data = await response.json();
  ...
  const logo = "https://s2.coinmarketcap.com/static/img/coins/64x64/1.png";
  ...
  return (
    <CRow>
      <CRow>
        ...
        <CCol sm={8}>
          <PriceChart coinSymbol="BTCUSDT" coinData={coinData} />
        </CCol>
      </CRow>
          ...
          <CCard>
            <CCardBody>
              <div>
                ...
                <ClusterChart symbol="BTCUSDT" year={selectedYear} />
              </div>
            </CCardBody>
          </CCard>
        </CCol>
